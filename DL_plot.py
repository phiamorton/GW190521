...
plt.plot(D_L, kernel(D_L),linewidth=1, ls='--', color = 'steelblue', label='Discovery paper')
#plt.hist()
...
#fig = plot_median_cr(draws, label = 'D_{effective}', unit='Mpc', median_label='LVK old waveform')
plt.plot(D_L,draws_pdf,linewidth=1, color = 'mediumorchid', label='GWTC-2.1 (conditioned)')
...
#fig = plot_median_cr(draws, label = 'D_{effective}', unit='Mpc', median_label='LVK old waveform')
plt.plot(D_L,draws_pdf, linewidth=1, ls='-.', color = 'lightsalmon', label='GWTC-2.1 (marginal)')
perc = np.sum(draws_pdf[D_L<DL_em]*(D_L[1]-D_L[0]))
...
plt.axvline(DL_em, ls=':', linewidth=1.2, c='darkred', label='AGN J124942.3+3449')

plt.xlabel(r'$D_L$ [Mpc]')
plt.ylabel(r'$p(D_L)$')
plt.ylim(bottom=0., top = 0.00062)
plt.xlim(0.,9000)
plt.legend()

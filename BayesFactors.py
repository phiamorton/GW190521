...
with h5py.File('inference_noEM_plpk/raynest.h5', 'r') as f:
    ...
    no_EM_plpk_logZ_error=  np.array(f['combined']['logZ_error'])
    print(no_EM_plpk_logZ_error)
...
with h5py.File('inference_H/raynest.h5', 'r') as f:
    ...
    H_0_logZ_error=  np.array(f['combined']['logZ_error'])

with h5py.File('inference_H_GW17/raynest.h5', 'r') as f:
    H_0_withGW17 = np.array(f['combined']['logZ'])
    H_0_withGW17_error = np.array(f['combined']['logZ_error'])
...
print("\nLog Bayes Factor redshift model vs no EM association pl+pk= {:.1f}".format(redshift_logZ - no_EM_plpk_logZ),  redshift_logZ_error + no_EM_plpk_logZ_error ) #- np.log(13)) #if subtracting prior odds as in Ashton#- not / for log"
...
print("\nLog Bayes Factor redshift model with free H_0 vs no EM association pl+pk= {:.1f}".format(H_0_logZ - no_EM_plpk_logZ), H_0_logZ_error + no_EM_plpk_logZ_error ) #- np.log(13)) #if subtracting prior odds as in Ashton#- not / for log"
...
print("\nLog Bayes Factor redshift model with free H_0 and GW170817 vs no EM association pl+pk= {:.1f}".format( H_0_withGW17 - no_EM_plpk_logZ), H_0_withGW17_error + no_EM_plpk_logZ_error )  #- np.log(13)) #if subtracting prior odds as in Ashton#- not / for log"
...
if remove_prior:
    print("\nLog Bayes Factor redshift model vs no EM association pl+pk -Ashton prior={:.1f} ".format( redshift_logZ - no_EM_plpk_logZ -Ashton_prior), redshift_logZ_error + no_EM_plpk_logZ_error)#- np.log(13)) #if subtracting prior odds as in Ashton#- not / for log"

    print("\nLog Bayes Factor redshift model with free H_0 vs no EM association pl+pk -Asthon prior= {:.1f}".format(H_0_logZ - no_EM_plpk_logZ -Ashton_prior), H_0_logZ_error + no_EM_plpk_logZ_error) #- np.log(13)) #if subtracting prior odds as in Ashton#- not / for log"
    print("\nLog Bayes Factor redshift model with free H_0 and GW170817 vs no EM association pl+pk- Asthon prior= {:.1f}".format( H_0_withGW17 - no_EM_plpk_logZ -Ashton_prior), H_0_withGW17_error + no_EM_plpk_logZ_error)#- np.log(13)) #if subtracting prior odds as in Ashton#- not / for log"
